...
def add_silence_to_audio(audio_path, silence_duration=0.5):
    ...
    try:
        ...
        # 创建静音数组
        silence = np.zeros(silence_samples, dtype=wav_data.dtype)
        
        # 拼接静音和原音频
        wav_with_silence = np.concatenate([silence, wav_data])
        
        # 保存回文件
        sf.write(audio_path, wav_with_silence, sample_rate)
        
        print(f">> Successfully added {silence_duration}s silence to audio")
        return audio_path
